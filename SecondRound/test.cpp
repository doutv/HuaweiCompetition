...
vector<pair<int, double>> GUV[MAX_EDGE];
vector<pair<int, double>> GVU[MAX_EDGE];
int edge_size;
...
void dfs(int u, int depth, double prec)
{
    ...
    for (i = get_GUV_lower_bound(u); i < GUV[u].size(); i++)
    {
        ...
        if (frac < 0.2 || frac > 3)
            continue;
        if (bag2.find(v) != bag2.end() && visited[v] == 0 && depth <= 6)
        {
        ...
        if ((flag[v] != target && depth >= 5) || depth > 6)
            continue;
        if (!visited[v] && (depth <= 4 || flag[v] == target))
        {
            visited[v] = 1;
