...
const int ANS3_MAX = 10005;
const int ANS4_MAX = 10005;
const int ANS5_MAX = 10005;
const int ANS6_MAX = 10005;
const int ANS7_MAX = 20005;
#else
...
void reverse_dfs(int u, double nxtc)
{
    // 标记倒走3步以内能到达的点
    // nxtc--u--nowc1--v1--nowc2--v2--nowc3--v3
    register int i, j, k;
    int v1, v2, v3;
    double nowc1, nowc2, nowc3;
    double frac;
    for (i = get_GVU_lower_bound(u); i < GVU[u].size(); i++)
    {
        // 反向第一层
        v1 = node_hashmap[GVU[u][i].first];
        if (!in_degree[v1] || !out_degree[v1])
            continue;
        nowc1 = GVU[u][i].second;
        frac = nxtc / nowc1;
        if (frac < 0.2 || frac > 3)
            continue;
        if (!visited[v1])
